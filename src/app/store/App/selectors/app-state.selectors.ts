...
export const flightInfo: (state: IAppStore) => string = (state: IAppStore) => state.appState.authToken;
// endregion
...
export const selectFlightInfo: any = createSelector(authToken, (value: string) => value);
// endregion
...
export class AppStateSelectors implements IAppStateSelectors {
    ...
    flightInfo$: Observable<string> = this.store.pipe(select(selectAuthToken));
}
