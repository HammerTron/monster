 ...
 */
import { IConnectionError } from './types/connection-error.model';
import { IErrorTrigger } from './types/error-trigger.model';
import { IMockOptions } from './types/mock-options.model';
 ...
 */
const errorTriggers: Array<IErrorTrigger> = [];
 ...
 */
const errors: Array<IConnectionError> = [
    {
...
function evaluateTriggers(data: any, method: string) {
    ...
    errorTriggers
        .filter(trigger => trigger.method === method.toLowerCase())
        .forEach(trigger => {
            if (trigger.fn(data)) {
...
/**
 * Set whether or not APIMock should randomly produce server errors
 *
...
function getErrorByCode(errorCode: number) {
    const error = errors.filter(errorItem => errorItem.status === errorCode);
 ...
 */
/* eslint-disable */
function randomError(errorArray: Array<any>) {
    let error, weightedSum;

    errorArray = errorArray || errors;

    const totalWeight = errorArray.reduce((acc: number, value: IConnectionError) => acc + value.distribution, 0);

    if (totalWeight > MAX_ERROR_DISTRIBUTION) {
        throw new Error('Sum of distributions is greater than defined max');
    }

    const randomNumber = randomInt(0, MAX_ERROR_DISTRIBUTION);
    error = undefined;
    weightedSum = 0;
    ...
    if (!nextError) {
        if (!errorsDisabled) {
            ...
            for (let i = 0, errorLength = errorArray.length; i < errorLength; i += 1) {
    ...
    if (error) {
        console.log('generated error: ' + JSON.stringify(error));
    }
...
/**
 * Helper function that APIMock uses to determine if an error is to
...
function produceError(data: any, method: string, errorArray?: any) {
    let error: any = evaluateTriggers(data, method);
 ...
 *
 * @param forceErrors http error code to force as the randomly generated error
 * @param enableRandomErrors flag used to turn on/off random error generation from mocking layer
 */
function setRandomErrorsConfig(enableRandomErrors: boolean, forceErrors?: any) {
